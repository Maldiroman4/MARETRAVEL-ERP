  ...
  closeNd(ndId) {
    ...
    Object.values(providerCosts).forEach((group, idx) => {
      ...
      const newNc = {
        ...
        serviceCategory: (nd.items && nd.items[0]?.serviceType) || nd.serviceType || window.state?.servicioActivo || 'BOLETO_AEREO',
        serviceType: (nd.items && nd.items[0]?.serviceType) || nd.serviceType || window.state?.servicioActivo || 'BOLETO_AEREO',
        servicio_tipo: (nd.items && nd.items[0]?.serviceType) || nd.serviceType || window.state?.servicioActivo || 'BOLETO_AEREO',
        createdById: data.currentUser.id,
        createdAt: new Date().toLocaleString()
      };
