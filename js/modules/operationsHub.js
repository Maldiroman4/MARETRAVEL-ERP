        ...
        if (hasUsdItem || (isLikelyUsdAmount && itemMatchesUsd)) {
          ...
          (data.creditNotes || []).forEach(nc => {
            if (nc.originDebitNoteId === nd.id || nc.originDebitNoteNumber === nd.ndNumber) {
              ...
              const originService = (nd.items && nd.items[0]?.serviceType) || nd.serviceType || nd.serviceCategory;
              if (originService && (!nc.servicio_tipo || !nc.serviceType)) {
                nc.servicio_tipo = nc.servicio_tipo || originService;
                nc.serviceType = nc.serviceType || originService;
                nc.serviceCategory = nc.serviceCategory || originService;
                changed = true;
              }
            }
          });
