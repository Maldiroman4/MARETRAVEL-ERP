...
function ymd(date: Date | string | undefined | null): string {
  if (!date) return '';
...
function renderOfficialPrintDocument(
  ...
  const docNumber = escapeHtml(
    doc.ndNumber || doc.ncNumber || doc.number || doc.id || '4001',
  );
  const emissionDate = formatDate(doc.issueDate);
  const emissionTime = doc.issueTime || '10:30';
  const solicitante = escapeHtml(
    doc.solicitante || doc.requestedBy || 'GERENCIA GENERAL',
  ).toUpperCase();
  const entityCode = escapeHtml(
    doc.accountCode ||
      doc.clientCode ||
      doc.providerCode ||
      (isNc ? 'PRV-001' : 'CLI-001'),
  );
  const entityName = escapeHtml(
    doc.accountName ||
      doc.clientName ||
      doc.providerName ||
      'CLIENTE / PROVEEDOR',
  ...
  const tcUsed =
    toNumber(doc.frozenExchangeRate ?? doc.exchangeRateUsed) || DEFAULT_RATE;
  const currencyStr = doc.currency || 'BOB';
  const amountBob = toNumber(doc.totalAmountBob);
  const amountUsd = toNumber(doc.totalAmountUsd);

  let totalUsd: string;
  let totalBob: string;
  if (currencyStr === 'USD') {
    totalUsd = (amountUsd || amountBob / tcUsed).toFixed(2);
    totalBob = (amountBob || amountUsd * tcUsed).toFixed(2);
  } else {
    totalBob = (amountBob || amountUsd * tcUsed).toFixed(2);
    totalUsd = (amountUsd || amountBob / tcUsed).toFixed(2);
  }
      ...
      : [
          {
            ...
            description:
              doc.concept || doc.observations || 'SERVICIO REGISTRADO',
  ...
  const itemsHtml = items
    .map((it: Record<string, any>, idx: number) => {
      ...
      const desc = escapeHtml(
        it.description || 'SERVICIO REGISTRADO',
      ).toUpperCase();
      const provName = escapeHtml(
        it.providerName || 'OPERADOR REGISTRADO',
      ).toUpperCase();
      const itemSubtotal = toNumber(it.subtotal ?? it.total).toFixed(2);
  ...
  const obsText = escapeHtml(
    doc.observations || doc.concept || 'OPERACIÓN OFICIAL REGISTRADA EN ERP',
  ).toUpperCase();
...
export class PrintService {
  ...
  private loadLogo(): string {
    ...
    candidates.push(join(process.cwd(), 'assets', 'logo.jpg'));
    candidates.push(join(process.cwd(), 'assets', 'logo.png'));
    let dir = __dirname;
    for (let i = 0; i < 8 && dir.startsWith(root); i++) {
      candidates.push(join(dir, 'assets', 'logo.jpg'));
      candidates.push(join(dir, 'assets', 'logo.png'));
    ...
    for (const candidate of candidates) {
      if (existsSync(candidate)) {
        const mime =
          extname(candidate).toLowerCase() === '.png' ? 'png' : 'jpeg';
        return `data:image/${mime};base64,${readFileSync(candidate).toString('base64')}`;
      }
    }
    return '';
  }

  async render(docType: 'ND' | 'NC', id: string): Promise<string> {
    const type = String(docType).toUpperCase();
    if (type !== 'ND' && type !== 'NC') {
      throw new BadRequestException('Tipo de documento inválido');
    }

    const logoBase64 = this.loadLogo();
    ...
    if (type === 'ND') {
      ...
      return renderOfficialPrintDocument(
        {
          ndNumber: nd.ndNumber,
          issueDate: ymd(nd.issueDate),
          solicitante:
            nd.requesterText || nd.passengerName || 'GERENCIA GENERAL',
          clientCode: nd.client?.code,
          clientName: nd.client?.name,
          currency: nd.currency,
          totalAmountBob: Number(nd.totalAmountBob),
          totalAmountUsd: Number(nd.totalAmountUsd),
          observations: nd.observations,
          items: nd.items.map((item) => ({
            serviceType: item.serviceType,
            description: item.description,
            providerName: 'OPERADOR REGISTRADO',
            providerCode: item.operatorId || 'PRV-001',
            subtotal: Number(item.totalAmount),
            currency: item.currency,
            serviceDetails: {
              travelDates: ymd(nd.issueDate),
              pnrCode: item.ticketNumber,
    ...
    return renderOfficialPrintDocument(
      {
        ...
        issueDate: ymd(nc.createdAt),
        concept: nc.concept,
        providerCode: nc.provider?.code,
        providerName: nc.provider?.name,
        currency: nc.currency,
        totalAmountBob: Number(nc.totalAmountBob),
        totalAmountUsd: Number(nc.totalAmountUsd),
      },
      'NC',
      logoBase64,
    );
