...
describe('CashReceiptsService', () => {
  ...
  it('void restores ND balances/status and marks the receipt REVERSADO with motivo', async () => {
    ...
    prisma.cashReceipt.update.mockImplementation(({ data }: any) =>
    ...
    expect(prisma.cashReceipt.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'cr1' },
        data: expect.objectContaining({
          status: 'REVERSADO',
  ...
  it('void on a missing receipt throws BadRequestException', async () => {
    prisma.cashReceipt.findUnique.mockResolvedValue(null);
